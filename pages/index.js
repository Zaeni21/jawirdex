...
export default function Home() {
  ...
  return (
    <main className="p-4 space-y-10 max-w-md mx-auto">
      <h1 className="text-2xl font-bold mb-4">JWR Mini DEX</h1>
      ...
      {!account ? (
        ...
        <>
          <div className="bg-white p-4 rounded shadow">
            ...
            <input type="text" placeholder="Amount JWR" className="w-full p-2 border mb-2" value={amountA} onChange={e => setAmountA(e.target.value)} />
            <input type="text" placeholder="Amount USDT" className="w-full p-2 border mb-2" value={amountB} onChange={e => setAmountB(e.target.value)} />
            <button onClick={approveAndAddLiquidity} className="bg-green-600 text-white px-4 py-2 rounded w-full">Add Liquidity</button>
          </div>

          <div className="bg-white p-4 rounded shadow mt-6">
            <h2 className="text-xl font-semibold mb-2">Swap</h2>
            <select className="w-full p-2 border mb-2" onChange={e => setSwapFrom(e.target.value)}>
              <option value="">Select Token From</option>
              <option value={JWR_TOKEN}>JWR</option>
              <option value={USDT_TOKEN}>USDT</option>
            </select>
            <select className="w-full p-2 border mb-2" onChange={e => setSwapTo(e.target.value)}>
              <option value="">Select Token To</option>
              <option value={JWR_TOKEN}>JWR</option>
              <option value={USDT_TOKEN}>USDT</option>
            </select>
            <input type="text" placeholder="Amount to Swap" className="w-full p-2 border mb-2" value={swapAmount} onChange={e => setSwapAmount(e.target.value)} />
            <button onClick={approveAndSwap} className="bg-purple-600 text-white px-4 py-2 rounded w-full">Swap</button>
          </div>
        </>
      )}
    </main>
